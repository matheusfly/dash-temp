...
import { Teacher, ScheduleEntry, ClassType } from '../types.ts';
import { TrashIcon, PlusIcon, InformationCircleIcon, ClockIcon, CalendarIcon, RectangleStackIcon, UsersIcon, CalendarDaysIcon, UserGroupIcon, ClipboardIcon, QuestionMarkCircleIcon } from './icons.tsx';
import WheelTimePickerPopover from './WheelTimePickerPopover.tsx';
        ...
        <div className="grid grid-cols-2 gap-4">
            <div className="relative">
                ...
                <select name="classType" value={formData.classType} onChange={handleChange} className={`${baseInputClasses} pl-12`}>
                    {(Object.values(ClassType) as ClassType[]).filter(t => t !== ClassType.PONTO && t !== ClassType.SUPERVISAO_LIVRE).map(type => <option key={type} value={type}>{type}</option>)}
                </select>
            </div>
            <div className="relative">
